...
#remove outlier -- do not run; for testing effect of outlier
#fig6_cd3 <- fig6_cd3[-4,]
...
```{r Generate monolayer titer figures (Sup_2)}
...
ggsave(width = 3, height = 4,"../results/sup_2.png")
```

```{r Generate humoral titer figures (Sup_5)}
...
ggsave(width = 6, height = 4,"../results/sup_5.png")
```

```{r Generate objective response t-test (Sup_6)}
...
ggsave(width = 4.5, height = 4.5, "../results/sup_6.png")
```
...
#run multiple univariate analysis
...
#                              colData = meta,
#                              design= ~ batch + status) #-- do not run; here for testing purposes

#dds <- DESeqDataSetFromMatrix(countData = nano,
#                              colData = meta,
#                              design= ~ batch + type) #-- do not run; here for testing purposes

dds <- DESeqDataSetFromMatrix(countData = nano,
                              colData = meta,
                              design= ~ batch + grade) #formula used for manuscript

#dds <- DESeqDataSetFromMatrix(countData = nano,
#                              colData = meta,
#                              design= ~ batch + sex) #-- do not run; here for testing purposes

dds <- DESeq(dds)


resName <- resultsNames(dds)
#res <- lapply(resName, function(x) results(dds, name = x)) #-- do not run
res <- results(dds, name = resName[3])
#res <- results(dds, name = resName[3], pAdjustMethod = "none", alpha = 0.05, lfcThreshold = 0.58) #-- do not run
#lapply(res, summary) #-- do not run
summary(res)
res
...
ggsave(width = 8, height = 5, "../results/sup_7a.png")

#extract upregaulted features for pathway analysis
...
pathsUpWdeseq <- pathAnalysis@result$Description[1:8]
ggsave(width = 10, height = 5, "../results/sup_7b.png")
```


```{r alternative nanoString analysis to confirm conclusions}
...
meta <- as.data.frame(lapply(meta, as.factor))
rownames(nano) <- genes
nano <- tibble::rownames_to_column(nano, "gene")
colnames(nano) <- c("gene",as.character(meta$name))
nano <- melt(nano)
nano$value <- as.numeric((nano$value))
nano <- nano %>% left_join(meta[c("name","status","sex","grade")],by = c("variable" = "name"))

#check normality -- not normally distibuted
normal_test_data <- nano %>%
  dplyr::group_by(gene) %>% 
...
if (nrow(not_normal_distribution) > 0) {
  data_for_Wilcox <- left_join(not_normal_distribution, nano) 
  
  significant_Wilcox <- data_for_Wilcox %>%
    group_by(gene) %>% 
    do(tidy(wilcox.test(.$value~.$grade,exact=FALSE))) %>%
    filter(p.value < 0.05)
}
  
directionChange <- data_for_Wilcox %>%
  group_by(gene,grade) %>% summarize(avgVal = mean(value))

#extract genes up in high grade
directionChange <- directionChange[directionChange$gene %in% significant_Wilcox$gene,]
directionChange <- directionChange %>% spread(key = grade, value = avgVal) %>% mutate(directionChange = bhigh/alow)
upFeats <- directionChange[directionChange$directionChange >1,]

#compile for plot
wilcox_for_plot <- left_join(significant_Wilcox, nano)

#plot the data
p <- ggplot(wilcox_for_plot, aes(grade, value)) +
  geom_boxplot(aes(color = gene)) +
...
#complete pathway analysis for genes up in high grade
mart <- useDataset("hsapiens_gene_ensembl", useMart("ensembl"))
G_list <- getBM(filters = "hgnc_symbol", attributes = c("ensembl_gene_id", "entrezgene_id", "external_gene_name", "description"), values = upFeats$gene, mart = mart)
G_list <- G_list[!duplicated(G_list$entrezgene_id),]
...
#list upregualted pathways
pathAnalysis@result$Description[1:28]

#check which pathways overlap with those idenitified using DEseq2
pathsUpWdeseq[pathsUpWdeseq %in% pathAnalysis@result$Description[1:28]]
```
...
ggsave(width = 6, height = 3,"../results/sup_8.png")
